...
func (JobSpyAdapter) Kind() model.SourceKind { return model.SourceKindSidecar }

func (a JobSpyAdapter) baseURL(config map[string]any) string {
	return strutil.StringOr(config["url"], a.URL)
}

func (a JobSpyAdapter) Search(ctx context.Context, query model.SearchQuery, config map[string]any) ([]model.NormalizedJob, error) {
	site := "linkedin"
