...
func (d Source) HealthCheck(ctx context.Context, _ map[string]any) (bool, error) {
...
func (d Source) fetchPage(ctx context.Context, url string, headers map[string]string) (string, error) {
	if d.Retrieval != nil {
		result, err := d.Retrieval.Fetch(ctx, ports.FetchRequest{URL: url, Headers: headers})
		if err != nil {
			return "", err
		}
		if result.Outcome.Status == ports.PageChallenged {
			return "", fmt.Errorf("wellfound: challenged: %s", result.Outcome.Reason)
		}
		if result.Outcome.Status == ports.PageRefused {
			return "", fmt.Errorf("wellfound: refused: %s", result.Outcome.Reason)
		}
		if result.Outcome.Status == ports.PageDeferred {
	...
	if retrieval.DefaultDetector.IsChallenged(html, 0) {
...
func (d Source) FetchDetail(ctx context.Context, jobURL string, _ map[string]any) (WellfoundDetailPatch, error) {
